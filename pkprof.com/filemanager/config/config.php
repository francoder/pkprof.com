 ...
 ******************************************************************************/
// Запоминаем текущий каталог (<public_dir>/filemanager)
$fm_dir = getcwd(); 
chdir('../../');
...
$dotenv->overload();
$dotenv->required(['APP_FOLDER', 'BIGSITER_CORE_FOLDER']);
define('APP_FOLDER',			getenv('APP_FOLDER'));
define('BIGSITER_CORE_FOLDER',	getenv('BIGSITER_CORE_FOLDER'));

$f3 = \Base::instance();
if (!$f3->exists('COOKIE.user_sign', $sign)) {
    $f3->error(401);
}

// Загружаем конфигурационные файлы
$f3->config(BIGSITER_CORE_FOLDER . '/cfg/config.ini', true);
$f3->config(APP_FOLDER . '/cfg/site.ini');

// Определяем текущую локаль
if ($f3->exists('SESSION.language', $lang) && is_array($f3->get('site.languages')) && in_array($lang, $f3->get('site.languages'))) {
	$f3->set('LANGUAGE',$lang);
} else {
    $f3->copy('FALLBACK', 'LANGUAGE');
}

// Определяем локаль для файл-менеджера
$lang = $f3->get('i18n.locale');
$lang2 = $lang.'_'. strtoupper($lang);
if (file_exists($fm_dir.'/lang/'.$lang.'.php')) {
    $default_language = $lang;
} elseif (file_exists($fm_dir.'/lang/'.$lang2.'.php')) {
    $default_language = $lang2;
} else {
    $default_language = 'en_EN';
}
// Возвращаем значение текущего каталога
chdir($fm_dir);
// =============================================================================
...
setlocale(LC_CTYPE, $f3->get('i18n.locale')); //correct transliteration
...
define('DEBUG_ERROR_MESSAGE', $f3->get('DEBUG')>0?true:false); // TRUE or FALSE
...
$config = array(
	...
	*/
	...
	'access_keys' => array(md5($sign.$_SESSION['secret_salt'])),
	...
	*******************/
	...
	'aviary_language'                         => $f3->get('i18n.locale'),
	'aviary_theme'                            => "light",
	'aviary_tools'                            => "all",
	'aviary_maxSize'                          => "1400",
